...
module.exports = async function autoRespostaCommand(sock, from, Info, prefix, BOT_PHONE, args) {
    try {
        ...
        if (args.length === 0) {
            return sock.sendMessage(from, {
                text: `🤖 *CONFIGURAÇÃO DE RESPOSTA AUTOMÁTICA*\n\n` +
                      ...
                      `💡 *Exemplo:* ${prefix}auto oi Seja bem-vindo ao grupo!`
            }, { quoted: Info });
        }
        ...
        if (!response) {
            return sock.sendMessage(from, { text: `❌ Você precisa definir uma resposta para a palavra *"${keyword}"*.\n\nExemplo: ${prefix}auto ${keyword} Olá, tudo bem?` }, { quoted: Info });
        }
